...
- AI: `POST /v1/ai/generate`, `POST /v1/ai/plan-chat`, calendar: `POST /v1/ai/calendar` (SSE chunked stream with keep-alive heartbeats + final summary/done meta), `POST /v1/ai/calendar/save` (persist manual edits), `GET /v1/ai/calendar/{calendarId}` (reload saved entries). All calendar operations persist runs/entries to Supabase for diffable history.
